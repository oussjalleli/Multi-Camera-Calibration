...
from multical.my_threading import cpu_count
...
class Camera(Parameters):
  ...
  def calibrate(boards, detections, image_size, max_iter=10, eps=1e-3,
    ...
    object_points = [np.array(pts, dtype=np.float32).reshape(-1, 1, 3) for pts in points.object_points]
    image_points = [np.array(corners, dtype=np.float32).reshape(-1, 1, 2) for corners in points.corners]

    # Call cv2.calibrateCamera
