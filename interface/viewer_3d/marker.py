...
Projects the corners of a virtual image plane using the camera's intrinsic matrix.
The projected corners are then scaled by a factor (size) and returned.
projection_corners(camera, scale=1):

Computes the corners of the image plane for a given camera and scale.
axis_points(scale=1):

Generates the points of the axis markers in 3D space.
axis_marker(scale=1):

Creates a PyVista mesh representing the axis markers with given scale.
image_projection(camera, distance=1):

Projects the corners of a virtual image plane and creates a PyVista mesh with UV coordinates.
scaled(pose, scale):

Scales the pose by the specified factor.
Classes:
Marker:

Represents a marker in the visualization scene.
Initializes with a PyVista mesh, pose, options, and scale.
Provides methods to set color, opacity, ambient lighting, point size, and visibility.
SceneMeshes:

Contains PyVista meshes for boards, cameras, and axes in the scene.
Provides methods to update camera scale and scene based on calibration results.
AxisSet:

Manages a set of axis markers in the scene.
Initializes with PyVista meshes representing axes and poses.
Provides methods to update poses and show/hide markers.
CameraSet:

Manages a set of camera markers in the scene.
Initializes with PyVista meshes representing cameras and poses.
Provides methods to update poses, highlight active cameras, and show/hide markers.
BoardSet:

Manages a set of board markers in the scene.
Initializes with PyVista meshes representing boards, colors, and poses.
Provides methods to update poses, highlight active boards, and show/hide markers.
Other:
camera_colors: A struct containing colors for different camera states."""